...
def clone(item):
    return GroceryItem(item.name, item.base_price, item.image_path)
...
def setup_belt():
    ...
    front_x = scanner_x - NEAR_SCANNER_GAP

    if count == 1:
        positions = [front_x]
    else:
        usable_space = front_x - LEFT_LIMIT
        gap_that_fits = usable_space / (count - 1)
        gap = max(MIN_ITEM_GAP, min(MAX_ITEM_GAP, gap_that_fits))
        start_x = front_x - (count - 1) * gap

        if start_x < LEFT_LIMIT:
            start_x = LEFT_LIMIT

        positions = [start_x + i * gap for i in range(count)]

    for item, x in zip(current_cart, positions):
        belt_objects.append({
            "item": item,
            "x": x,
            "y": 95,
...
def new_customer():
    global current_customer, customer_done, scanned_items, payment_done
...
def draw_item(o):
    ...
    conveyor.create_rectangle(
        x - ITEM_HALF_SIZE, y - ITEM_HALF_SIZE,
        x + ITEM_HALF_SIZE, y + ITEM_HALF_SIZE,
        fill="white"
    )
...
def animate():
    ...
    for o in belt_objects:
        if o["visible"]:
            o["x"] += belt_speed
            if o["x"] + ITEM_HALF_SIZE >= scanner_x:
                process(o)
...
ttk.Button(frame, text="Start Belt", command=start_belt).grid(row=0, column=0, padx=5, pady=5)
ttk.Button(frame, text="Stop Belt", command=stop_belt).grid(row=0, column=1, padx=5, pady=5)
ttk.Button(frame, text="Next Customer", command=new_customer).grid(row=1, column=0, padx=5, pady=5)
ttk.Button(frame, text="Checkout", command=checkout).grid(row=1, column=1, padx=5, pady=5)
ttk.Button(frame, text="Finish Customer", command=finish_customer).grid(row=2, column=0, padx=5, pady=5)
ttk.Button(frame, text="Clock Out", command=clock_out).grid(row=2, column=1, padx=5, pady=5)
